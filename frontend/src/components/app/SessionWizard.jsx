  ...
  return (
    <Dialog open={open}>
      <DialogContent
        ...
        hideClose
      >
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.25em] text-[#FF5F15] font-bold">
          <span>Start New Trip</span>
          <span className="text-neutral-500">· Step {step} / 3</span>
        </div>
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              {["Previous", "New", "Future"].map((t) => (
                <button
                  ...
                  className={`h-20 rounded-sm border text-sm font-bold uppercase tracking-wider transition-colors ${
                    sessionType === t
                      ? "bg-[#FF5F15] border-[#FF5F15] text-white"
                      : "bg-[#0A0A0A] border-[#262626] text-neutral-300 hover:border-[#FF5F15]"
                  }`}
                >{t}</button>
              ))}
            </div>
            ...
            {sessionType && sessionType !== "New" && (
              <div className="space-y-1.5">
                <Label className="text-xs uppercase tracking-wider text-neutral-400">Select Date</Label>
                <Popover>
                  <PopoverTrigger asChild>
                    <Button data-testid="session-date-btn" variant="outline"
                      className="w-full h-12 justify-start bg-[#0A0A0A] border-[#262626] text-white rounded-sm hover:bg-[#262626]">
            ...
            <div className="flex gap-2 pt-2">
              <Button variant="outline" onClick={onCancel}
                className="h-12 rounded-sm bg-[#0A0A0A] border-[#262626] text-white hover:bg-[#262626]">Cancel</Button>
              <Button data-testid="wizard-next-1" disabled={!sessionType} onClick={next}
                className="flex-1 h-12 rounded-sm bg-[#FF5F15] hover:bg-[#E04F0E] text-white font-bold">
          ...
          <div className="space-y-4 mt-2">
            ...
            {loadType === "Warehouse" && (
              <div className="space-y-2">
                <Label className="text-xs uppercase tracking-wider text-neutral-400">Dairy or Water?</Label>
                <div className="grid grid-cols-2 gap-2">
                  {["Dairy", "Water"].map((t) => (
                    <button
                      key={t}
                      ...
                      className={`h-16 rounded-sm border text-sm font-bold uppercase tracking-wider transition-colors ${
                        subType === t
                          ? "bg-[#FF5F15] border-[#FF5F15] text-white"
                          : "bg-[#0A0A0A] border-[#262626] text-neutral-300 hover:border-[#FF5F15]"
                      }`}
            ...
            <div className="flex gap-2 pt-2">
              <Button variant="outline" onClick={back}
                className="h-12 rounded-sm bg-[#0A0A0A] border-[#262626] text-white hover:bg-[#262626]">
        ...
        {step === 3 && (
          <div className="space-y-4 mt-2">
            ...
            <div className="space-y-3">
              <div className="p-3 bg-[#0A0A0A] border border-[#262626] rounded-sm">
                <div className="text-[10px] uppercase tracking-wider text-neutral-500 mb-1">Driver ID (from profile)</div>
                <div data-testid="wizard-driverid" className="font-bold">{profile?.driver_id}</div>
              </div>

              {profile?.truck_assignment_type === "Slip Seat" && (
                <div>
                  <Label className="text-xs uppercase tracking-wider text-neutral-400">Truck # (Slip Seat) *</Label>
                  <Input data-testid="wizard-truck" value={truckNumber} onChange={(e) => setTruckNumber(e.target.value)}
                    className="bg-[#0A0A0A] border-[#262626] text-white h-12 rounded-sm mt-1.5" />
                </div>
              )}

              <div>
                <Label className="text-xs uppercase tracking-wider text-neutral-400">Order # *</Label>
                <Input data-testid="wizard-order" value={orderNumber} onChange={(e) => setOrderNumber(e.target.value)}
                  className="bg-[#0A0A0A] border-[#262626] text-white h-12 rounded-sm mt-1.5" />
              </div>
              <div>
                <Label className="text-xs uppercase tracking-wider text-neutral-400">BOL # *</Label>
